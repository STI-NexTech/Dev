	...
	internal class ActivityNextStepQueryExecutor: INextStepQueryExecutor
	{
		...
		private Dictionary<string, object> GetAdditionalInfo(Entity entity) {
			...
			if (categoryId.IsNotEmpty()) {
				additionalInfo["ActivityCategoryId"] = categoryId;
				var isNotProcessElemet = entity.GetTypedColumnValue<Guid>("ProcessElementId").IsEmpty();
				if (isNotProcessElemet) {
					additionalInfo["UserTaskEntitySchemaName"] = "Activity";
				}
			}
